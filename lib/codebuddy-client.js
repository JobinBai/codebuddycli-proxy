...
const INTERNAL_BASE_URL = 'https://copilot.tencent.com/v2';
const EXTERNAL_BASE_URL = 'https://www.codebuddy.ai/v2';

/** 与 CLI 的产品配置选择一致：internal / ioa 均使用国内 Copilot 端点。 */
function defaultBaseUrlForEnvironment(environment = process.env.CODEBUDDY_INTERNET_ENVIRONMENT) {
  const normalized = typeof environment === 'string' ? environment.trim().toLowerCase() : '';
  return normalized === 'internal' || normalized === 'ioa' ? INTERNAL_BASE_URL : EXTERNAL_BASE_URL;
}
...
function resolveEndpoint(baseUrl) {
  return normalizeChatCompletionsUrl(baseUrl || process.env.CODEBUDDY_BASE_URL || defaultBaseUrlForEnvironment());
}
...
class CodebuddyClient {
   ...
   * @param {string}  [opts.baseUrl]            显式上游地址，优先级最高
