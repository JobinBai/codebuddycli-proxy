...
// 作为 X-IDE-Version 的默认值。当前对齐 Agent SDK 0.3.235 内置的 CLI 2.130.0。
const CLI_VERSION = process.env.CODEBUDDY_CLI_VERSION || '2.130.0';

// OpenAI Node SDK 的客户端指纹。version 需与 CLI 内置的 openai 依赖一致。
const STAINLESS_PACKAGE_VERSION = '6.25.0';
// OpenAI SDK 的默认超时为 10 分钟；它会将该值（秒）写入请求头。
const STAINLESS_TIMEOUT_SECONDS = 600;
// ModelProvider 删除 OpenAI SDK 预置的 User-Agent 后，Axios Node adapter 补入此值。
const AXIOS_USER_AGENT = 'axios/1.18.1';
 ...
 * @param {number}  [opts.timeoutSeconds] x-stainless-timeout，默认 600
...
function buildHeaders(opts) {
  const {
    ...
    privateData = false,
    retryCount = 0,
    timeoutSeconds = STAINLESS_TIMEOUT_SECONDS,
    extraHeaders,
  } = opts;
  ...
  const headers = {
    ...
    'x-stainless-timeout': String(timeoutSeconds),
  ...
  // 这不是 CLI/<version>：ModelProvider 会删除 OpenAI SDK 的 UA，
  // 随后的 Axios Node adapter 以默认值补上 axios/<version>。
